...
def get_args():
    ...
    parser.add_argument('--n_rnn', help='Number of layers in the stacked RNN',
            type=check_positive, choices=range(1,12), required=True)
    ...
    parser.add_argument('--batch_size', help='size of mini-batch',
           type=check_positive, choices=range(1,10000), required=True)
    ...
    tag = functools.reduce(lambda a, b: a+b, sys.argv).replace('--resume', '').replace('/', '-').replace('--', '-').replace('True', 'T').replace('False', 'F')
...
while True:
    ...
    if total_iters % 500 == 0:
        print(total_iters, total_iters + 1)
        total_iters += 1
    try:
